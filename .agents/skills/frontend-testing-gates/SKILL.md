...
- **Hooks livianos**: Los hooks de Git (`.githooks/`, instalados mediante `npm run delivery:hooks:install`) nunca ejecutan suites. Para humanos son consultivos respecto de evidencia y CI; con `DELIVERY_REQUIRE_EVIDENCE=1`, `pre-commit` y `pre-push` fallan cerrado y exigen receipt, ledger, ventana CI y reparación válida. El guard del cliente sólo anticipa feedback. Para los agentes, `delivery_prepare` es la entrada obligatoria previa a cada commit.
