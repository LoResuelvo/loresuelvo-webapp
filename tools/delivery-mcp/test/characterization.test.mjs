...
test("characterization: selección de gate por impacto Cucumber sobre steps existentes y hooks (patrón 5d0a34d)", async () => {
  ...
  // 3. Step existente utilizado por una sola feature -> Gate B
  ...
  assert.strictEqual(resultShared.gate.id, "B");
  assert.strictEqual(resultShared.status, "ready");
  assert.ok(resultShared.gate.reasonCodes.includes("SINGLE_FEATURE_STEP_CONSUMER"));
  assert.strictEqual(resultShared.impact.gate, "B");
  assert.strictEqual(
    resultShared.gate.parameters.featureFile,
