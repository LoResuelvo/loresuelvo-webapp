...
```text
delivery_test({ mode: "affected" | "unit" | "scenario" | "diagnostic", ... }) # TDD focalizado en ciclo RED/GREEN
...
delivery_job_wait({ jobId, timeoutMs })                        # espera no bloqueante de jobs largos (Gate D/R o CI)
...
El agente edita y usa Git normalmente, pero no ejecuta manualmente `make`, lint, typecheck, suites ni comandos crudos de CI como flujo habitual. Durante el ciclo RED/GREEN utiliza exclusivamente `delivery_test` para iteración rápida sin generar receipts de commit. En la frontera atómica utiliza `delivery_prepare` sobre el snapshot staged. Ante gates largos (Gate D o Gate R), `delivery_prepare` y `delivery_finalize` operan en modo job devolviendo un `jobId` que se aguarda de forma no bloqueante con `delivery_job_wait`. El servidor MCP devuelve resultados procesados compactos. Ante indisponibilidad o timeout de un job, se exige recuperación o escalación; queda prohibido el bypass con CLI o comandos directos.
