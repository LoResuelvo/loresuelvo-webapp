...
6. El hook `pre-push` valida el receipt de Gate R o el contexto humano exacto, comprueba que el SHA objetivo coincida con el fallo activo y consume la autorización (de uso único). `DELIVERY_REQUIRE_EVIDENCE=1` continúa bloqueando la ruta humana `not_run`.
...
- **Hooks de Git livianos**: `pre-commit`, `commit-msg`, `post-commit` y `pre-push` **nunca ejecutan suites de tests**.
  ...
  - `post-commit`: si hubo un receipt coincidente, lo consume y registra el commit como `passed`; si no hubo receipt, lo registra como `not_run`. Solo conserva y consume un contexto humano `repair_ci` cuando parent, branch, árbol y mensaje coinciden exactamente.
