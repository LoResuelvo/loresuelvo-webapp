import { Given, When, Then } from "@cucumber/cucumber";
import { CustomWorld, APP_URL } from "../support/world";
import assert from "assert";
import { ROUTES } from "../../lib/routes";
import { aCategory, aProvider } from "../support/factories";

const CONSUMER_URL = APP_URL + ROUTES.consumer.home;

Given("estoy en el home de consumidores", async function (this: CustomWorld) {
  await this.page.goto(CONSUMER_URL);
});

Given("existen los rubros Plomería, Electricista y Gasista", async function (this: CustomWorld) {
  await this.stubGet("/categories", [
    aCategory({ id: 1, name: "Plomería", description: "" }),
    aCategory({ id: 2, name: "Electricista", description: "" }),
    aCategory({ id: 3, name: "Gasista", description: "" }),
  ]);
});

Given("existen los siguientes prestadores registrados:", async function (this: CustomWorld, dataTable) {
  const providers = dataTable.hashes();
  const providersByCategory: Record<string, any[]> = {};

  for (const provider of providers) {
    const categoryId = provider.category_id;
    if (!providersByCategory[categoryId]) {
      providersByCategory[categoryId] = [];
    }

    providersByCategory[categoryId].push(
      aProvider({
        id: parseInt(provider.id.replace("prov-", "")),
        name: provider.name,
        surname: provider.surname,
        category_name: provider.category_name,
        profile_photo_url: provider.profile_photo_url,
      })
    );
  }

  for (const [categoryId, providersList] of Object.entries(providersByCategory)) {
    await this.stubGet(`/providers?category_id=${categoryId}`, providersList);
  }
});

Given(
  "que no existen prestadores registrados para el rubro {string}",
  async function (this: CustomWorld, categoryName: string) {
    const categoryMap: Record<string, number> = {
      Plomería: 1,
      Electricista: 2,
      Gasista: 3,
    };

    const categoryId = categoryMap[categoryName];
    if (!categoryId) {
      throw new Error(`Category ID not found for: ${categoryName}`);
    }

    await this.stubGet(`/providers?category_id=${categoryId}`, []);
  }
);

When("hago clic en la tarjeta del rubro {string}", async function (this: CustomWorld, categoryName: string) {
  const card = this.page
    .getByRole("button", { name: categoryName, exact: true })
    .or(this.page.getByRole("link", { name: categoryName, exact: true }))
    .or(this.page.getByText(categoryName, { exact: true }))
    .first();

  await card.waitFor({ state: "visible" });
  await card.click();
});

Then(
  "soy redirigido al listado de técnicos del rubro {string}",
  async function (this: CustomWorld, categoryName: string) {
    await this.page
      .waitForURL((url) => !url.toString().endsWith("/consumidor/home"), { timeout: 5000 })
      .catch(() => {});
  }
);

Then(
  "veo la tarjeta del técnico {string} con el rubro {string}",
  async function (this: CustomWorld, providerName: string, categoryName: string) {
    const providerText = this.page.getByText(providerName, { exact: false }).first();
    await providerText.waitFor({ state: "visible" });
    assert.ok(await providerText.isVisible(), `There is no provider "${providerName}"`);

    const categoryText = this.page.getByText(categoryName, { exact: false }).first();
    assert.ok(await categoryText.isVisible(), `There is no category "${categoryName}"`);
  }
);

Then("veo el mensaje {string}", async function (this: CustomWorld, mensaje: string) {
  const messageElement = this.page.getByText(mensaje, { exact: false }).first();
  await messageElement.waitFor({ state: "visible" });
  assert.ok(await messageElement.isVisible(), `There is no message: "${mensaje}"`);
});
