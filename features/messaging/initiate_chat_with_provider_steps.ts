...
Given("que estoy buscando prestadores por rubro", async function (this: CustomWorld) {
  ...
  if (!(await this.hasApiStub("GET", "/categories"))) {
    await this.stubGet("/categories", [
      aCategory({ id: 1, name: "Plomería", description: "Servicios de plomería" }),
      aCategory({ id: 2, name: "Electricidad", description: "Servicios eléctricos" }),
    ]);
  }
...
Given("que ya envié un mensaje a un prestador", async function (this: CustomWorld) {
  ...
  if (!(await this.hasApiStub("GET", "/categories"))) {
    await this.stubGet("/categories", [aCategory({ id: 1, name: "Plomería", description: "Servicios de plomería" })]);
  }
