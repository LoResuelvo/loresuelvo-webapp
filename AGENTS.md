...
- **Git hooks**: los hooks de Git (`pre-commit`, `commit-msg`, `post-commit`, `pre-push`) son deliberadamente livianos. **Nunca ejecutan suites de tests**. `commit-msg` valida el formato del mensaje (`<type>: description` o `<type>[XX]: description`). Para desarrolladores humanos, `pre-commit`, `post-commit` y `pre-push` operan de forma consultiva (`advisory`) sin bloquear el commit ni el push. Se instalan una vez por clon con `npm run delivery:hooks:install`.
