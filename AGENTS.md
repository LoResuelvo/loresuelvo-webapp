...
- **Agentes autónomos**: ejecutar el cliente con `DELIVERY_REQUIRE_EVIDENCE=1`. MCP `delivery_prepare` es obligatorio sobre el snapshot staged antes de cada commit. El guard anticipatorio da feedback temprano; `pre-commit` y `pre-push` son las barreras locales fail-closed. Si el MCP requerido no está disponible, detenerse; la CLI neutral (`npm run delivery:*`) solo sustituye al MCP en un entorno aprobado explícitamente o para uso humano.
- **Humanos**: desarrollan, realizan stage y pueden commitear directamente ejecutando tests de forma manual o delegando la verificación a CI. En ausencia de receipt previo, los hooks operan en modo consultivo (`advisory`) informando el estado sin bloquear el commit ni el push.
- **Git hooks**: son livianos y **nunca ejecutan suites**. `commit-msg` valida el formato. En modo humano, `pre-commit` y `pre-push` son consultivos; `post-commit` registra `not_run` si falta receipt. Con `DELIVERY_REQUIRE_EVIDENCE=1`, `pre-commit` exige receipt válido y `pre-push` exige ledger/evidencia válidos, un solo commit, ventana CI libre y reparación Gate R ligada al incidente activo; una autorización se consume en la aprobación local, antes de conocer el resultado remoto. `DELIVERY_SKIP_CI_CHECK` se rechaza en ambos modos. Se instalan una vez por clon con `npm run delivery:hooks:install`.
- **Ventana de CI sin polling del developer**: La ventana continua de hasta cuatro commits en vuelo y los incidentes activos se evalúan automáticamente por `delivery_prepare`. El desarrollador no debe realizar polling ni monitorear periódicamente CI por SHA tras cada push.
- **Flujo de reparación de CI (`repair_ci`)**: Los agentes requieren Gate R ante fallos de CI, que reproduce los checks de CI asignados a agentes y emite un receipt de reparación. Los humanos pueden corregir y pushear directamente dado que los hooks son consultivos.
