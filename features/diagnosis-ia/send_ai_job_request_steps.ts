...
import { aAiConversation, aAiConversationDetail, anAiMessage, anApiError } from "../support/factories";
...
Given("la evaluación del chatbot todavía requiere más información", async function (this: CustomWorld) {
  ...
  const prompt = "¿Podrías describir con más detalle dónde se produce la pérdida?";
  await this.stubGet("/chatbot/conversations", [
    aAiConversation({
      last_message: anAiMessage({ id: 2, sender_role: "chatbot", content: prompt, created_on: "2026-06-18T10:00:01Z" }),
    }),
  ]);
  ...
  await this.stubGet(
    ...
    aAiConversationDetail({
      ...
      messages: [
        anAiMessage({ id: 1, sender_role: "consumer", content: "Hay un problema con el agua", created_on: "2026-06-18T10:00:00Z" }),
        anAiMessage({ id: 2, sender_role: "chatbot", content: prompt, created_on: "2026-06-18T10:00:01Z" }),
      ],
...
Given(
  ...
  async function (this: CustomWorld) {
    ...
    const prompt = "Podés resolverlo vos mismo ajustando el cuerito de la canilla.";
    await this.stubGet("/chatbot/conversations", [
      aAiConversation({
    ...
    await this.stubGet(
      ...
      aAiConversationDetail({
        title: "Canilla que gotea",
        assessment: { outcome: "self_service", problem_category: { id: 1, name: "Plomería" } },
        messages: [
          anAiMessage({ id: 1, sender_role: "consumer", content: "La canilla gotea", created_on: "2026-06-18T10:00:00Z" }),
          anAiMessage({ id: 2, sender_role: "chatbot", content: prompt, created_on: "2026-06-18T10:00:01Z" }),
        ],
        recommended_providers: [],
      })
...
Given("el servicio de solicitudes de trabajo no está disponible", async function (this: CustomWorld) {
  await this.stubPost("/chatbot/conversations/1/job-requests", 500, anApiError("Internal Server Error"));
...
Given("ya existe una solicitud de trabajo abierta con {string}", async function (this: CustomWorld, providerName: string) {
  const errorMsg = `Ya existe una solicitud de trabajo abierta con ${providerName}`;
  await this.stubPost("/chatbot/conversations/1/job-requests", 409, anApiError(errorMsg));

  await this.page.route("**/chatbot/conversations/1/job-requests", async (route) => {
    await route.fulfill({
      status: 409,
      contentType: "application/json",
      body: JSON.stringify(anApiError(errorMsg)),
    });
  });
});
...
Then("el sistema envía la solicitud de trabajo al prestador {string}", async function (this: CustomWorld, providerName: string) {
  ...
  await successMessage.waitFor(visibleTimeout);
...
Then("veo una confirmación de que la solicitud fue enviada", async function (this: CustomWorld) {
  ...
  await confirmation.waitFor(visibleTimeout);
...
Then("soy redirigido a la conversación de trabajo con {string}", async function (this: CustomWorld, providerName: string) {
  await this.page.waitForURL(`**${ROUTES.consumer.messages}**`, waitTimeout);
  assert.ok(
...
Then("veo un mensaje de error indicando que no se pudo enviar la solicitud", async function (this: CustomWorld) {
  ...
  await errorMessage.waitFor(visibleTimeout);
...
Then("veo un mensaje indicando que ya existe una solicitud abierta con ese prestador", async function (this: CustomWorld) {
  ...
  await duplicateMessage.waitFor(visibleTimeout);
